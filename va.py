...
# ==========================================
# GŁÓWNY KOD WIZYJNY (BAZA: pol.py)
# ==========================================
...
ERROR_MESSAGES_SIDE = {
    ...
    "piszczel": "Piszczele powinny być ustawione w miarę pionowo.",
    "noga_w_gorze": "Pociągnij podniesioną nogę wyżej.",
...
def draw_side_skeleton(frame, p, checks, metrics, check_phase):
    ...
    if check_phase in (3, 4): kolano_ok = checks.get("kolano_podporowe", True)
...
def main():
    ...
    while True:
        ...
        if ok_front:
            view_front, ok_front_status, checks_front = process_front_view(frame_front, pose_front, check_phase,
                                                                           skeleton_only=skeleton_only)
        else:
        ...
        if overall_ok:
            ...
            if frames_held >= current_required:
                if state == 1:
                    ...
                    assistant.speak("Dobra robota, podnieś prawą nogę.")
                    state = 3
                elif state == 3:
            ...
            if consecutive_errors > ERROR_THRESHOLD and error_cooldown == 0:
                ...
                if len(checks_side) > 0 and 0 < len(failed_side) <= 2:
